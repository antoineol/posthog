...
-- Full CREATE schema for the prod-eu/data node. Apply to a fresh ClickHouse to build it.
...
CREATE TABLE posthog.asyncdeletion (
  ...
  team_id UInt64
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/noshard/asyncdeletion', '{replica}-{shard}') ORDER BY (deletion_type, delete_verified_at) PARTITION BY toYYYYMM(created_at) SETTINGS index_granularity = 8192;
...
CREATE TABLE posthog.metrics_query_log (
  host String,
  timestamp DateTime,
  ...
  log_comment String,
  is_exception UInt8
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/noshard/posthog.metrics_query_log', '{replica}-{shard}') ORDER BY (toDate(timestamp), team_id, query_type) PARTITION BY toYYYYMM(timestamp) SETTINGS index_granularity = 8192;
...
CREATE TABLE posthog.new_writable_raw_sessions (
  team_id Int64,
  session_id_v7 UInt128,
  distinct_id AggregateFunction(argMax, String, DateTime64(6, 'UTC')),
  min_timestamp SimpleAggregateFunction(min, DateTime64(6, 'UTC')),
...
CREATE TABLE posthog.pending_person_deletes_reporting (
  ...
  team_id Int64
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/pending_person_deletes_reporting', '{shard}-{replica}') ORDER BY (team_id, deletion_type, key) SETTINGS index_granularity = 8192;
...
CREATE TABLE posthog.person_collapsing (
  ...
  _offset UInt64,
  _sign Int8
) ENGINE = ReplicatedCollapsingMergeTree('/clickhouse/prod/tables/noshard/posthog.person_collapsing', '{replica}-{shard}', _sign) ORDER BY (team_id, id) SETTINGS index_granularity = 8192;
...
CREATE TABLE posthog.plugin_log_entries (
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/posthog.plugin_log_entries.fixed', '{replica}-{shard}', _timestamp) ORDER BY (team_id, plugin_id, plugin_config_id, timestamp) PARTITION BY toYYYYMMDD(timestamp) TTL toDate(timestamp) + toIntervalWeek(1) SETTINGS index_granularity = 512;
...
CREATE TABLE posthog.sharded_events (
  ...
  $window_id String DEFAULT replaceRegexpAll(JSONExtractRaw(properties, '$window_id'), '^"|"$', '') COMMENT 'column_materializer::$window_id',
  $session_id String DEFAULT replaceRegexpAll(JSONExtractRaw(properties, '$session_id'), '^"|"$', '') COMMENT 'column_materializer::$session_id',
  _timestamp DateTime,
  _offset UInt64,
  inserted_at Nullable(DateTime64(6, 'UTC')) DEFAULT now64(),
  person_mode Enum8('full'=0, 'propertyless'=1, 'force_upgrade'=2),
  mat_metadata_backend String DEFAULT replaceRegexpAll(JSONExtractRaw(properties, 'metadata.backend'), '^"|"$', ''),
  mat_metadata_loggedIn String DEFAULT replaceRegexpAll(JSONExtractRaw(properties, 'metadata.loggedIn'), '^"|"$', ''),
  ...
  INDEX minmax_mat_metadata_backend mat_metadata_backend TYPE minmax GRANULARITY 1,
  INDEX minmax_mat_metadata_loggedIn mat_metadata_loggedIn TYPE minmax GRANULARITY 1,
...
CREATE TABLE posthog.sharded_session_recording_events (
  ...
  click_count Int8 MATERIALIZED length(arrayFilter(x -> ((JSONExtractInt(x, 'type') = 3) AND (JSONExtractInt(x, 'data', 'source') = 2) AND (JSONExtractInt(x, 'data', 'source') = 2)), events_summary)),
  ...
  timestamps_summary Array(DateTime64(6, 'UTC')) MATERIALIZED arraySort(arrayMap(x -> toDateTime(JSONExtractInt(x, 'timestamp') / 1000), events_summary)),
  first_event_timestamp DateTime64(6, 'UTC') MATERIALIZED arrayReduce('min', timestamps_summary),
  last_event_timestamp DateTime64(6, 'UTC') MATERIALIZED arrayReduce('max', timestamps_summary),
  urls Array(String) MATERIALIZED arrayFilter(x -> (x != ''), arrayMap(x -> JSONExtractString(x, 'data', 'href'), events_summary))
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/{shard}/posthog.session_recording_events', '{replica}', _timestamp) ORDER BY (team_id, toHour(timestamp), session_id, timestamp, uuid) PARTITION BY toYYYYMMDD(timestamp) TTL toDate(created_at) + toIntervalWeek(3) SETTINGS index_granularity = 512;
...
CREATE TABLE posthog.swap_person_distinct_id (
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedCollapsingMergeTree('/clickhouse/prod/tables/noshard/posthog.swap_person_distinct_id', '{replica}-{shard}', _sign) ORDER BY (team_id, distinct_id, person_id) SETTINGS index_granularity = 8192;
...
CREATE TABLE posthog.web_pre_aggregated_bounces (
  ...
  total_session_count_state AggregateFunction(sum, UInt64),
  mat_metadata_loggedIn Nullable(Bool)
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/noshard/posthog.web_pre_aggregated_bounces', '{replica}-{shard}') ORDER BY (team_id, period_bucket, host, device_type, entry_pathname, end_pathname, browser, os, viewport_width, viewport_height, referring_domain, utm_source, utm_medium, utm_campaign, utm_term, utm_content, country_code, city_name, region_code, region_name, has_gclid, has_gad_source_paid_search, has_fbclid) PARTITION BY toYYYYMMDD(period_bucket) SETTINGS index_granularity = 8192;
...
CREATE MATERIALIZED VIEW posthog.session_replay_features_mv TO posthog.writable_session_replay_features (session_id String, team_id Int64, distinct_id String, min_first_timestamp DateTime64(6, 'UTC'), max_last_timestamp DateTime64(6, 'UTC'), event_count Int64, mouse_position_count Int64, mouse_sum_x Float64, mouse_sum_x_squared Float64, mouse_sum_y Float64, mouse_sum_y_squared Float64, mouse_distance_traveled Float64, mouse_direction_change_count Int64, mouse_velocity_sum Float64, mouse_velocity_sum_of_squares Float64, mouse_velocity_count Int64, scroll_event_count Int64, total_scroll_magnitude Float64, scroll_direction_reversal_count Int64, rapid_scroll_reversal_count Int64, click_count Int64, keypress_count Int64, mouse_activity_count Int64, rage_click_count Int64, dead_click_count Int64, inter_action_gap_count Int64, inter_action_gap_sum_ms Float64, inter_action_gap_sum_of_squares_ms Float64, max_idle_gap_ms Float64, quick_back_count Int64, page_visit_count Int64, unique_url_count AggregateFunction(uniqExactArray, Array(String)), console_error_count Int64, console_error_after_click_count Int64, network_request_count Int64, network_failed_request_count Int64, network_request_duration_sum Float64, network_request_duration_sum_of_squares Float64, network_request_duration_count Int64, max_scroll_y Float64, unique_click_target_count AggregateFunction(uniqExactArray, Array(Int64)), text_selection_count Int64, is_deleted UInt8) AS SELECT
  session_id,
  team_id,
...
CREATE TABLE posthog.events (
  ...
  inserted_at Nullable(DateTime64(6, 'UTC')) DEFAULT now64(),
  person_mode Enum8('full'=0, 'propertyless'=1, 'force_upgrade'=2),
  mat_metadata_backend String COMMENT 'column_materializer::properties::metadata.backend',
  mat_metadata_loggedIn String COMMENT 'column_materializer::properties::metadata.loggedIn',
