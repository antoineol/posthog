...
-- Full CREATE schema for the prod-us/data node. Apply to a fresh ClickHouse to build it.
...
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/posthog.adhoc_events_deletion', '{replica}-{shard}', deleted_at, is_deleted) ORDER BY (team_id, uuid) TTL deleted_at + toIntervalMonth(3) WHERE is_deleted = 1 SETTINGS default_compression_codec = 'lz4', index_granularity = 8192;
CREATE TABLE posthog.ai_events (
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('ai_events', 'posthog', 'ai_events', rand());
CREATE TABLE posthog.async_deletion_run (
  id UInt64,
  deletion_type UInt8,
  key String,
...
CREATE TABLE posthog.asyncdeletion (
  id UInt64,
  deletion_type UInt8,
  key String,
  group_type_index UInt64,
  created_at DateTime,
  delete_verified_at DateTime,
  created_by_id UInt64,
  team_id UInt64
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/noshard/asyncdeletion', '{replica}-{shard}') ORDER BY (deletion_type, delete_verified_at) PARTITION BY toYYYYMM(created_at) SETTINGS default_compression_codec = 'lz4', index_granularity = 8192;
CREATE TABLE posthog.asyncdeletion_join (
  id UInt64,
  deletion_type UInt8,
  key String,
  group_type_index UInt64,
  ...
  created_by_id UInt64,
  team_id UInt64
) ENGINE = Join(ANY, LEFT, team_id, key);
...
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/noshard/posthog.channel_definition', '{replica}-{shard}') ORDER BY (domain, kind) SETTINGS default_compression_codec = 'lz4', index_granularity = 8192;
...
CREATE TABLE posthog.cohortpeople (
  ...
  sign Int8,
  version UInt64
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/posthog.cohortpeople_new_partitioned', '{replica}-{shard}', version) ORDER BY (team_id, cohort_id, person_id) PARTITION BY team_id % 16 SETTINGS index_granularity = 8192;
...
CREATE TABLE posthog.distributed_posthog_document_embeddings (
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('posthog', 'posthog', 'sharded_posthog_document_embeddings', cityHash64(document_id));
...
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/noshard/posthog.duplicate_events', '{replica}-{shard}') ORDER BY (team_id, distinct_id, event, inserted_at) PARTITION BY toYYYYMMDD(inserted_at) TTL inserted_at + toIntervalDay(7) SETTINGS default_compression_codec = 'lz4', index_granularity = 512;
...
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/posthog.error_tracking_issue_fingerprint_overrides', '{replica}-{shard}', version) ORDER BY (team_id, fingerprint) SETTINGS default_compression_codec = 'lz4', index_granularity = 512;
...
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/posthog.new_events_dead_letter_queue_2', '{replica}-{shard}', _timestamp) ORDER BY (id, event_uuid, distinct_id, team_id) TTL toDate(_timestamp) + toIntervalWeek(4) SETTINGS default_compression_codec = 'lz4', index_granularity = 512;
...
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/posthog.exchange_rate', '{replica}-{shard}', version) ORDER BY (date, currency) SETTINGS default_compression_codec = 'lz4', index_granularity = 8192;
...
CREATE TABLE posthog.ingestion_warnings (
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = Distributed('aux', 'posthog', 'sharded_ingestion_warnings', rand());
...
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/noshard/posthog.llma_metrics_daily', '{replica}-{shard}') ORDER BY (team_id, date, metric_name) PARTITION BY toYYYYMM(date) SETTINGS default_compression_codec = 'lz4', index_granularity = 8192;
...
CREATE TABLE posthog.new_raw_sessions (
  team_id Int64,
...
CREATE TABLE posthog.new_sessions (
  session_id String,
  team_id Int64,
...
CREATE TABLE posthog.pending_person_deletes_reporting (
  id UInt64,
  deletion_type UInt8,
  key String,
  ...
  team_id Int64
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/pending_person_deletes_reporting', '{shard}-{replica}') ORDER BY (team_id, deletion_type, key) SETTINGS default_compression_codec = 'lz4', index_granularity = 8192;
...
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/am0005_20220623103132_noshard/posthog.person', '{replica}-{shard}', version) ORDER BY (team_id, id) SETTINGS index_granularity = 8192;
CREATE TABLE posthog.person_collapsing (
  id UUID,
  created_at DateTime64(3),
  team_id Int64,
  properties String,
  ...
  _offset UInt64,
  _sign Int8
) ENGINE = ReplicatedCollapsingMergeTree('/clickhouse/prod/tables/noshard/posthog.person_collapsing', '{replica}-{shard}', _sign) ORDER BY (team_id, id) SETTINGS default_compression_codec = 'lz4', index_granularity = 8192;
CREATE TABLE posthog.person_distinct_id (
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedCollapsingMergeTree('/clickhouse/tables/noshard/posthog.person_distinct_id-swap2-2021-08-18', '{replica}-{shard}', _sign) ORDER BY (team_id, distinct_id, person_id) SETTINGS default_compression_codec = 'lz4', index_granularity = 8192;
...
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/posthog.person_distinct_id_overrides', '{replica}-{shard}', version) ORDER BY (team_id, distinct_id) SETTINGS default_compression_codec = 'lz4', index_granularity = 512;
CREATE TABLE posthog.person_distinct_id_overrides_join_table (
  team_id Int64,
  distinct_id String,
  person_id UUID
) ENGINE = Join(ANY, LEFT, team_id, distinct_id);
CREATE TABLE posthog.person_distinct_id_overrides_to_delete (
  team_id Int64,
  distinct_id String,
  version Int64
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/posthog.person_distinct_id_overrides_to_delete', '{replica}-{shard}', version) ORDER BY (team_id, distinct_id) SETTINGS default_compression_codec = 'lz4', index_granularity = 512;
CREATE TABLE posthog.person_distinct_id_overrides_to_delete_join (
  team_id Int64,
  distinct_id String,
  version Int64
) ENGINE = Join(ANY, LEFT, team_id, distinct_id);
CREATE TABLE posthog.person_overrides (
  ...
  version Int32
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/posthog.person_overrides', '{replica}-{shard}', version) ORDER BY (team_id, old_person_id) PARTITION BY toYYYYMM(oldest_event) SETTINGS default_compression_codec = 'lz4', index_granularity = 8192;
CREATE TABLE posthog.person_overrides_join_table (
  team_id Int64,
  distinct_id String,
  person_id UUID
) ENGINE = Join(ANY, LEFT, team_id, distinct_id);
CREATE TABLE posthog.person_overrides_to_delete (
  team_id Int64,
  distinct_id String,
...
CREATE TABLE posthog.person_static_cohort (
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/prod/tables/noshard/person_static_cohort', '{replica}-{shard}', _timestamp) ORDER BY (team_id, cohort_id, person_id, id) SETTINGS default_compression_codec = 'lz4', index_granularity = 8192;
...
CREATE TABLE posthog.plugin_log_entries (
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/posthog.plugin_log_entries', '{replica}-{shard}', _timestamp) ORDER BY (team_id, plugin_id, plugin_config_id, timestamp) PARTITION BY toYYYYMMDD(timestamp) TTL toDate(timestamp) + toIntervalWeek(1) SETTINGS default_compression_codec = 'lz4', index_granularity = 512;
CREATE TABLE posthog.property_definitions (
  ...
  version UInt64 MATERIALIZED bitShiftLeft(toUInt64(NOT (property_type IS NULL)), 48) + toUInt64(toUnixTimestamp(last_seen_at))
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/posthog.property_definitions', '{replica}-{shard}', version) ORDER BY (team_id, type, coalesce(event, ''), name, coalesce(group_type_index, 255)) SETTINGS always_fetch_merged_part = 0, default_compression_codec = 'lz4', index_granularity = 8192, replicated_can_become_leader = 1;
...
CREATE TABLE posthog.session_recording_events (
  ...
  timestamps_summary Array(DateTime64(6, 'UTC')) COMMENT 'column_materializer::timestamps_summary',
  first_event_timestamp DateTime64(6, 'UTC') COMMENT 'column_materializer::first_event_timestamp',
  last_event_timestamp DateTime64(6, 'UTC') COMMENT 'column_materializer::last_event_timestamp',
  urls Array(String) COMMENT 'column_materializer::urls'
) ENGINE = Distributed('posthog', 'posthog', 'sharded_session_recording_events', sipHash64(distinct_id));
...
CREATE TABLE posthog.sessions (
  ...
  pageview_count SimpleAggregateFunction(sum, Int64),
  autocapture_count SimpleAggregateFunction(sum, Int64)
) ENGINE = Distributed('sessions', 'posthog', 'sessions', sipHash64(session_id));
...
CREATE TABLE posthog.sharded_events (
  uuid UUID,
  event String,
  properties String CODEC(Default),
  timestamp DateTime64(6, 'UTC'),
  team_id Int64,
  distinct_id String,
  ...
  _offset UInt64,
  elements_chain String,
  $group_0 String DEFAULT replaceRegexpAll(JSONExtractRaw(properties, '$group_0'), concat('^[', regexpQuoteMeta('"'), ']*|[', regexpQuoteMeta('"'), ']*$'), ''),
  $group_1 String DEFAULT replaceRegexpAll(JSONExtractRaw(properties, '$group_1'), concat('^[', regexpQuoteMeta('"'), ']*|[', regexpQuoteMeta('"'), ']*$'), ''),
  $group_2 String DEFAULT replaceRegexpAll(JSONExtractRaw(properties, '$group_2'), concat('^[', regexpQuoteMeta('"'), ']*|[', regexpQuoteMeta('"'), ']*$'), ''),
  $group_3 String DEFAULT replaceRegexpAll(JSONExtractRaw(properties, '$group_3'), concat('^[', regexpQuoteMeta('"'), ']*|[', regexpQuoteMeta('"'), ']*$'), ''),
  $group_4 String DEFAULT replaceRegexpAll(JSONExtractRaw(properties, '$group_4'), concat('^[', regexpQuoteMeta('"'), ']*|[', regexpQuoteMeta('"'), ']*$'), ''),
  $window_id String DEFAULT replaceRegexpAll(JSONExtractRaw(properties, '$window_id'), concat('^[', regexpQuoteMeta('"'), ']*|[', regexpQuoteMeta('"'), ']*$'), ''),
  $session_id String DEFAULT replaceRegexpAll(JSONExtractRaw(properties, '$session_id'), concat('^[', regexpQuoteMeta('"'), ']*|[', regexpQuoteMeta('"'), ']*$'), ''),
  person_id UUID,
  person_properties String CODEC(Default),
  ...
  properties_group_custom Map(String, String) MATERIALIZED mapSort(mapFilter((key, _) -> ((key NOT LIKE '$%') AND (key NOT IN ('token', 'distinct_id', 'utm_source', 'utm_medium', 'utm_campaign', 'utm_content', 'utm_term', 'gclid', 'gad_source', 'gclsrc', 'dclid', 'gbraid', 'wbraid', 'fbclid', 'msclkid', 'twclid', 'li_fat_id', 'mc_cid', 'igshid', 'ttclid', 'rdt_cid'))), CAST(JSONExtractKeysAndValues(properties, 'String'), 'Map(String, String)'))) CODEC(Default),
  properties_group_feature_flags Map(String, String) MATERIALIZED mapSort(mapFilter((key, _) -> (key LIKE '$feature/%'), CAST(JSONExtractKeysAndValues(properties, 'String'), 'Map(String, String)'))) CODEC(Default),
  is_deleted Bool,
  person_properties_map_custom Map(String, String) MATERIALIZED mapSort(mapFilter((key, _) -> (key NOT LIKE '$%'), CAST(JSONExtractKeysAndValues(person_properties, 'String'), 'Map(String, String)'))) CODEC(Default),
  mat_$ai_trace_id Nullable(String) DEFAULT JSONExtract(properties, '$ai_trace_id', 'Nullable(String)'),
  ...
  properties_group_ai Map(String, String) MATERIALIZED mapSort(mapFilter((key, _) -> ((key LIKE '$ai_%') AND (key NOT IN ('$ai_input', '$ai_input_state', '$ai_output', '$ai_output_choices', '$ai_output_state', '$ai_tools'))), CAST(JSONExtractKeysAndValues(properties, 'String'), 'Map(String, String)'))) CODEC(Default),
  mat_historical_migration Nullable(String) DEFAULT JSONExtract(properties, 'historical_migration', 'Nullable(String)'),
  ...
  mat_$ai_experiment_id Nullable(String) MATERIALIZED JSONExtract(properties, '$ai_experiment_id', 'Nullable(String)'),
  ...
  INDEX minmax_mat_$ai_trace_id `mat_$ai_trace_id` TYPE minmax GRANULARITY 1,
  ...
  INDEX minmax_mat_historical_migration mat_historical_migration TYPE minmax GRANULARITY 1,
...
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/reshard3/{shard}/posthog.sharded_events', '{replica}', _timestamp) ORDER BY (team_id, toDate(timestamp), event, cityHash64(distinct_id), cityHash64(uuid)) PARTITION BY toYYYYMM(timestamp) SAMPLE BY cityHash64(distinct_id) SETTINGS index_granularity = 8192, max_avg_part_size_for_too_many_parts = 0;
...
CREATE TABLE posthog.sharded_heatmaps (
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/reshard/{shard}/posthog.heatmaps', '{replica}') ORDER BY (type, team_id, toDate(timestamp), current_url, viewport_width) PARTITION BY toYYYYMM(timestamp) TTL toDate(timestamp) + toIntervalDay(90) SETTINGS index_granularity = 8192;
CREATE TABLE posthog.sharded_log_entries (
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/reshard/{shard}/posthog.sharded_log_entries', '{replica}', _timestamp) ORDER BY (team_id, log_source, log_source_id, instance_id, timestamp) PARTITION BY toYYYYMMDD(timestamp) TTL toDate(timestamp) + toIntervalDay(90) SETTINGS index_granularity = 1024, ttl_only_drop_parts = 1;
CREATE TABLE posthog.sharded_performance_events (
  ...
  _offset UInt64,
  _partition UInt64
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/reshard/{shard}/posthog.performance_events', '{replica}') ORDER BY (team_id, toDate(timestamp), session_id, pageview_id, timestamp) PARTITION BY toYYYYMM(timestamp) TTL toDate(timestamp) + toIntervalWeek(3) SETTINGS index_granularity = 8192;
CREATE TABLE posthog.sharded_person (
  id UUID,
  created_at DateTime,
  team_id Int64,
  properties String,
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/reshard/{shard}/posthog.sharded_person', '{replica}', _timestamp) ORDER BY (team_id, id) SETTINGS index_granularity = 8192;
CREATE TABLE posthog.sharded_person_distinct_id (
  id Int64,
  distinct_id String,
  person_id UUID,
  team_id Int64,
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/reshard/{shard}/posthog.sharded_person_distinct_id', '{replica}', _timestamp) ORDER BY (team_id, distinct_id, person_id) SETTINGS index_granularity = 8192;
...
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/reshard/{shard}/posthog.sharded_posthog_document_embeddings_text_embedding_3_small_1536', '{replica}', inserted_at) ORDER BY (team_id, toDate(timestamp), product, document_type, rendering, cityHash64(document_id)) PARTITION BY toMonday(timestamp) TTL timestamp + toIntervalMonth(3) SETTINGS index_granularity = 512, ttl_only_drop_parts = 1;
...
) ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/reshard/{shard}/posthog.preaggregation_results', '{replica}') ORDER BY (team_id, job_id, time_window_start, breakdown_value) PARTITION BY toYYYYMM(time_window_start) TTL expires_at SETTINGS index_granularity = 8192;
...
) ENGINE = ReplicatedAggregatingMergeTree('/clickhouse/tables/reshard/{shard}/posthog.raw_sessions_v3', '{replica}') ORDER BY (team_id, session_timestamp, session_id_v7) PARTITION BY toYYYYMM(session_timestamp) SETTINGS index_granularity = 8192;
CREATE TABLE posthog.sharded_session_replay_embeddings (
  ...
  input String
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/reshard/{shard}/posthog.session_replay_embeddings', '{replica}') ORDER BY (toDate(generation_timestamp), team_id, session_id) PARTITION BY toYYYYMM(generation_timestamp) TTL toDate(generation_timestamp) + toIntervalYear(1) SETTINGS index_granularity = 512;
...
CREATE TABLE posthog.swap_person_distinct_id (
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedCollapsingMergeTree('/clickhouse/prod/tables/noshard/posthog.person_distinct_id_', '{replica}-{shard}', _sign) ORDER BY (team_id, distinct_id, person_id) SETTINGS default_compression_codec = 'lz4', index_granularity = 8192;
...
) ENGINE = Distributed('ops', 'posthog', 'sharded_tophog', cityHash64(toString(key)));
...
CREATE TABLE posthog.web_bounces_daily_distributed (
...
) ENGINE = Distributed('posthog', 'posthog', 'web_bounces_daily', rand());
...
CREATE TABLE posthog.web_bounces_hourly_distributed (
  period_bucket DateTime,
  team_id UInt64,
  host String,
  device_type String,
  ...
  total_session_duration_state AggregateFunction(sum, Int64),
  total_session_count_state AggregateFunction(sum, UInt64)
) ENGINE = Distributed('posthog', 'posthog', 'web_bounces_hourly', rand());
...
CREATE TABLE posthog.web_pre_aggregated_bounces (
  ...
  total_session_count_state AggregateFunction(sum, UInt64),
  mat_metadata_loggedIn Nullable(Bool)
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/noshard/posthog.web_pre_aggregated_bounces', '{replica}-{shard}') ORDER BY (team_id, period_bucket, host, device_type, entry_pathname, end_pathname, browser, os, viewport_width, viewport_height, referring_domain, utm_source, utm_medium, utm_campaign, utm_term, utm_content, country_code, city_name, region_code, region_name, has_gclid, has_gad_source_paid_search, has_fbclid) PARTITION BY toYYYYMMDD(period_bucket) SETTINGS default_compression_codec = 'lz4', index_granularity = 8192;
...
CREATE TABLE posthog.web_stats_daily_distributed (
  ...
  sessions_uniq_state AggregateFunction(uniq, String),
  pageviews_count_state AggregateFunction(sum, UInt64)
) ENGINE = Distributed('posthog', 'posthog', 'web_stats_daily', rand());
...
CREATE TABLE posthog.web_stats_hourly_distributed (
  period_bucket DateTime,
  team_id UInt64,
  host String,
  device_type String,
  ...
  sessions_uniq_state AggregateFunction(uniq, String),
  pageviews_count_state AggregateFunction(sum, UInt64)
) ENGINE = Distributed('posthog', 'posthog', 'web_stats_hourly', rand());
...
CREATE TABLE posthog.writable_events_recent (
  ...
  _offset UInt64,
  inserted_at DateTime64(6, 'UTC') DEFAULT now64()
) ENGINE = Distributed('batch_exports_writable', 'posthog', 'sharded_events_recent', sipHash64(distinct_id));
...
CREATE TABLE posthog.writable_raw_sessions_primary (
  team_id Int64,
  session_id_v7 UInt128,
  distinct_id AggregateFunction(argMax, String, DateTime64(6, 'UTC')),
  min_timestamp SimpleAggregateFunction(min, DateTime64(6, 'UTC')),
...
CREATE TABLE posthog.writable_sessions (
  ...
  pageview_count SimpleAggregateFunction(sum, Int64),
  autocapture_count SimpleAggregateFunction(sum, Int64)
) ENGINE = Distributed('posthog_writable', 'posthog', 'sharded_sessions', sipHash64(session_id));
CREATE TABLE posthog.writable_sessions_background (
  session_id String,
  team_id Int64,
  distinct_id SimpleAggregateFunction(any, String),
  min_timestamp SimpleAggregateFunction(min, DateTime64(6, 'UTC')),
...
CREATE VIEW posthog.persons_properties_up_to_date_view AS SELECT *
...
CREATE VIEW posthog.persons_up_to_date_view AS SELECT
  id,
...
CREATE TABLE posthog.distributed_person (
  id UUID,
  created_at DateTime,
  team_id Int64,
  properties String,
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = Distributed('posthog', 'posthog', 'sharded_person', sipHash64(id));
CREATE TABLE posthog.distributed_person_distinct_id (
  id Int64,
  distinct_id String,
  person_id UUID,
  team_id Int64,
  _timestamp DateTime,
  _offset UInt64
) ENGINE = Distributed('posthog', 'posthog', 'sharded_person_distinct_id', sipHash64(distinct_id));
...
CREATE TABLE posthog.new_writable_sessions (
  session_id String,
  team_id Int64,
  distinct_id SimpleAggregateFunction(any, String),
  min_timestamp SimpleAggregateFunction(min, DateTime64(6, 'UTC')),
...
SETTINGS
  optimize_aggregation_in_order = 1;
CREATE VIEW posthog.events_with_array_props_view AS SELECT
  uuid,
  event,
  properties,
  timestamp,
...
CREATE VIEW posthog.team_events_last_month_view AS SELECT team_id, count() AS event_count
FROM posthog.events
WHERE
