...
-- Full CREATE schema for the dev/data node. Apply to a fresh ClickHouse to build it.
...
CREATE TABLE posthog.dmat_slot_assignments (
  team_id UInt64,
  column_index UInt8,
  property_name String,
  version UInt32 DEFAULT toUnixTimestamp(now())
) ENGINE = ReplacingMergeTree(version) ORDER BY (team_id, column_index) SETTINGS index_granularity = 8192;
CREATE TABLE posthog.dummy (
  id String
) ENGINE = MergeTree() ORDER BY (id) SETTINGS index_granularity = 8192;
CREATE TABLE posthog.dummy_privileges (
  id String
) ENGINE = MergeTree() ORDER BY (id) SETTINGS index_granularity = 8192;
...
CREATE TABLE posthog.groups2 (
  group_key String,
  created_at DateTime64(3)
) ENGINE = ReplicatedMergeTree('/clickhouse/tables/noshard/posthog.groups2', '{replica}-{shard}') ORDER BY (group_key) SETTINGS index_granularity = 8192;
...
CREATE TABLE posthog.kafka_error_tracking_issue_fingerprint_overrides_ws (
  ...
  is_deleted Int8,
  version Int64
) ENGINE = Kafka(warpstream_shared) SETTINGS kafka_format = 'JSONEachRow', kafka_group_name = 'clickhouse_error_tracking_issue_fingerprint_overrides_ws', kafka_topic_list = 'clickhouse_error_tracking_issue_fingerprint';
...
CREATE TABLE posthog.materialized_test (
  id UInt32,
  properties String
) ENGINE = MergeTree() ORDER BY (id) SETTINGS index_granularity = 8192;
...
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/am0005_20220705151225_noshard/posthog.person', '{replica}-{shard}', version) ORDER BY (team_id, id) SETTINGS index_granularity = 8192;
...
CREATE TABLE posthog.plugin_log_entries (
  ...
  _timestamp DateTime,
  _offset UInt64
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/posthog.plugin_log_entries', '{replica}-{shard}', _timestamp) ORDER BY (team_id, id) PARTITION BY plugin_id TTL toDate(timestamp) + toIntervalWeek(1) SETTINGS index_granularity = 512;
CREATE TABLE posthog.poc_events (
  uuid UUID,
  timestamp DateTime64(6, 'UTC'),
  team_id Int64,
  distinct_id String
) ENGINE = MergeTree() ORDER BY (team_id, toDate(timestamp), cityHash64(distinct_id), cityHash64(uuid)) PARTITION BY toYYYYMM(timestamp) SETTINGS index_granularity = 8192;
CREATE TABLE posthog.posthog_document_embeddings (
  team_id Int64,
  product LowCardinality(String),
  document_type LowCardinality(String),
  model_name LowCardinality(String),
...
) ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/noshard/posthog.posthog_document_embeddings', '{replica}-{shard}', inserted_at) ORDER BY (team_id, toDate(timestamp), product, document_type, model_name, rendering, cityHash64(document_id)) SETTINGS index_granularity = 512;
...
CREATE TABLE posthog.sharded_events_codec_test (
  ...
  INDEX minmax_$ai_experiment_id `mat_$ai_experiment_id` TYPE minmax GRANULARITY 1,
  INDEX minmax_$session_id_uuid `$session_id_uuid` TYPE minmax GRANULARITY 1
) ENGINE = ReplacingMergeTree(_timestamp) ORDER BY (team_id, toDate(timestamp), event, cityHash64(distinct_id), cityHash64(uuid)) PARTITION BY toYYYYMM(timestamp) SAMPLE BY cityHash64(distinct_id) SETTINGS default_compression_codec = 'ZSTD(3)', index_granularity = 8192;
