...
- **Title the observation, caption the count.** `title` says what was observed and for whom in one line a reader can act on, such as `Users who hit "Not found" opening a shared chat link`, not a label such as `Users affected`. `caption` says how the figure is counted, such as `Unique users on the captured exception, counted once per window`. The report shows both above the figure and names the query beneath it.
