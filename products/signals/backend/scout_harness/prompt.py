...
- **Title the observation, and caption only what the tile cannot show.** `title` says what was observed and for whom in one line a reader can act on, such as `Users who hit "Not found" opening a shared chat link`, not a label such as `Users affected`. The tile prints the figure with its `unit`, then the title, then the window the query covers, then any comparison. A reader sees them together, so never state one fact twice across them. Leave `caption` empty unless it carries something the reader needs and cannot see: a filter that narrows the count (`Production only, excluding internal users`), why a longer window was needed, or a caveat on the data (`Person context is missing on about a third of these events`). A caption that restates the title, the unit, or the window is noise.
...
- **Keep every metric live and bounded.** Give every metric one `InsightVizNode` wrapping a `TrendsQuery` you ran successfully this session. Every source series must be an `EventsNode` or `ActionsNode`. Give it a relative `dateRange.date_from` no longer than {MAX_LIVE_METRIC_WINDOW_DAYS} days, leave `date_to` empty, and keep the filters that reproduce the observation. Default the query to `dateRange.date_from: "-14d"` with `interval: "day"`. The inbox row draws the trailing 14 daily buckets as a strip beside the whole-window figure, so a 14-day daily window keeps the strip and the figure describing the same period. Use a longer window only when 14 days cannot show the observation, and use the caption to say why; the tile already prints the window itself. Its longitudinal output may contain at most {MAX_LIVE_METRIC_QUERY_POINTS} estimated interval points, including the current partial bucket. The query remains the source of truth when the report is reopened.
...
- **Keep semantics separate from formatting.** `kind` says what the metric measures. `value_format` says how to print it (`count`, `percentage`, `percentage_scaled`, `duration`, `currency`, or `number`), and `unit` is one lowercase word that completes the figure, because the report prints it next to the number: `users`, `sessions`, `events`, `runs`, or `calls`. For a rate, name what the share means: `failure` for an error rate, `conversion` for a conversion rate; `%` is redundant and is dropped. A duration keeps `ms` or `s`, and revenue keeps its ISO currency code. `affected_users` uses `count`. Use `percentage` for percentage points (`34` means 34%) and `percentage_scaled` for 0–1 ratios (`0.34` means 34%). A percentage query must set `aggregationAxisFormat` to exactly the same value as `value_format`; missing or numeric axis formatting is invalid.
...
- **A comparison is the same query over the previous window.** Run the query again over the 14 days before the window (`date_from: "-28d"` with `date_to: "-14d"` in that separate run only, because the stored query keeps an empty `date_to`), then send `comparison` with that value and a short label such as `Previous 14 days`. The report shows the change against it. Omit `comparison` rather than guessing a baseline.
