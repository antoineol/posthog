...
@pytest.mark.asyncio
@pytest.mark.parametrize("use_handoffs", [True, False])
async def test_batch_waits_for_every_grouped_signal_before_starting_summary(use_handoffs: bool) -> None:
    ...
    async def execute_activity(activity_fn: Callable[..., object], input: object, **kwargs: object) -> object:
        ...
        if activity_fn is assign_and_emit_signal_activity:
            ...
            return AssignAndEmitSignalOutput(
                ...
                run_count=1,
                signal_key="handoff-1" if len(assigned_inputs) == 1 and use_handoffs else None,
            )
    ...
    assert [signal.signal_id for signal in wait_inputs[0].signals] == [input.signal_id for input in assigned_inputs]
    assert wait_inputs[0].mode == WaitForClickHouseMode.CH_CONFIRMED
    assert wait_inputs[0].require_visible is use_handoffs

    if use_handoffs:
        assert activity_order == ["wait", "dispatch"]
        assert len(dispatch_inputs) == 1
        assert dispatch_inputs[0].signal_keys == ["handoff-1"]
