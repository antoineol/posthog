...
describe('list app tool call validation', () => {
    ...
    it('supports tools with different identifier names', () => {
        expect(() =>
            validateListAppToolCall(
                'trace-list',
                { detail_tool: 'trace-get', detail_args: '{ trace_id: item.trace_id }' },
    ...
    it('rejects an unknown target tool', () => {
        ...
        ).toThrow(/survey-list.*surveys-get.*not registered/)
    })
