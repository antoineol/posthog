...
export function validateListAppToolCall(appKey: string, config: ListAppCall, schema: z.ZodType | undefined): void {
    const context = `List app "${appKey}" calling "${config.detail_tool}"`
    if (!schema) {
        throw new Error(`${context}: target tool is not registered`)
    }
    ...
    const known = new Set(Object.keys(input.properties))
    const unknown = keys.filter((key) => !known.has(key))
    const missing = (input.required ?? []).filter((key) => !keys.includes(key))
