...
struct Cli {
    ...
    /// this run's ids, exiting nonzero on a mismatch that outlasts the
    ...
    /// Team owning the cohort. Without it, verify resolves the team from
    /// the rows themselves with a full-table scan.
    #[arg(long)]
    team_id: Option<i32>,
...
fn verify_config(cli: &Cli) -> Result<verify::VerifyConfig> {
    ...
    if cli.distinct_id_prefix.is_empty() {
        bail!("--verify requires a non-empty --distinct-id-prefix");
    }
    // Required: resolving the team from the rows would be a full-table scan.
...
mod tests {
    ...
    #[test]
    fn verify_config_refuses_an_empty_prefix() {
